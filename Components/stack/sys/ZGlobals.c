...
//uint16 zgConfigPANID;   //这里默认先不要数值，初始化给
// Device Logical Type
 ...
 * @fn          Setpanid
 *
 * @brief       Reads the ZCD_NV_PANID NV Item.and set the Item to PANID
 *
 * @param       none
 *
 * @return      none
...
void Setpanid(void)
{
  uint16 gu16RecBuffLen;
   ...
   if ( osal_nv_item_init( ZCD_NV_PANID,
                              2,
                              &gu16RecBuffLen ) == ZSUCCESS )
  
  {
    osal_nv_read(ZCD_NV_PANID,0,2,&gu16RecBuffLen);
  }
  zgConfigPANID=gu16RecBuffLen;
}
 ...
 * @fn          Initpanid
 *
 * @brief       If the PANID is not set by the user,use the default id
                in this function ,we set it 0x12ff.but you can change 
                you like.
 *
 * @param       none
 *
...
void Initpanid(void)
{
  uint16 gu16RecBuffLen=0xFFFF;       //默认PANID，自己可以任意修改
  uint16 panidbuffe;
  uint16 panidpre;                    //这个是指向ZCD_NV_PANID的值
  uint16 panid;                       //这个想ZCD_NV_PANID1的值
  
  osal_nv_read(ZCD_NV_PANID,0,2,&panidpre);
  osal_nv_read(ZCD_NV_PANID1,0,2,&panid);
  ...
  if(panidpre!=panid)
  {
   if ( osal_nv_item_init( ZCD_NV_PANID,
    ...
    {  
        osal_nv_write(ZCD_NV_PANID,0,2,&gu16RecBuffLen);
        osal_nv_read(ZCD_NV_PANID,0,2,&panidbuffe);
        zgConfigPANID=panidbuffe;
    } 
  }
}
